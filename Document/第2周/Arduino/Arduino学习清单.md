...
|------|------|
| 按钮的④脚 | Arduino **5V** |
| 按钮的②脚 | Arduino **2 号引脚** + **10kΩ 电阻 → GND**（下拉） |

> 按钮按下 → 2 号引脚读到 5V（HIGH）；松开 → 下拉电阻拉到 0V（LOW）
...
| 元件 | 连接 |
|------|------|
| LED 长脚(+) | **220Ω 电阻 → 13 号引脚** |
| LED 短脚(-) | Arduino **GND** |

> 220Ω 限流电阻保护 LED。13 号引脚板载 LED 也会同步闪。
...
|------|------|
| 按钮④脚 | Arduino **5V** |
| 按钮②脚 | Arduino **2 号引脚** + **10kΩ → GND**（下拉） |
| LED 长脚(+) | **220Ω → 13 号引脚** |
| LED 短脚(-) | Arduino **GND** |

> 01 + 02 的组合：按钮输入 + LED 输出。两条回路独立，靠代码关联。
...
| 元件 | 连接 |
|------|------|
| LED 长脚(+) | **220Ω → 9 号 PWM 引脚**（标有 ~） |
| LED 短脚(-) | Arduino **GND** |

> ⚠ 必须用带 `~` 的 PWM 引脚（UNO：3,5,6,9,10,11），普通数字引脚无效。
...
|------|------|
| 电位器脚①（左） | Arduino **5V** |
| 电位器脚②（中） | Arduino **A0**（模拟输入） |
| 电位器脚③（右） | Arduino **GND** |
| LED 长脚(+) | **220Ω → 9 号 PWM 引脚** |
| LED 短脚(-) | Arduino **GND** |

> 旋钮 → 电压变化 → analogRead(0~1023) → map(0~255) → LED 亮度。
...
> ⚠ 必须用**无源蜂鸣器**（底部无封胶）。有源蜂鸣器通电就响，无法用 `tone()` 控制音高。
